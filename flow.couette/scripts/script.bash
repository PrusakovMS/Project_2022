...
curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendMessage -d chat_id=${CHATID} -d text="Couette flow starting!
"
for temp in 0.5 1.0 1.5 2.0 2.5 3.0
do
	sed "s/YYYYTEMP/$temp/g" ./in.flow.couette > ./in.flow.couette_for_run
	srun -N 1 -p RT --ntasks-per-node=4 ~/bin/lmp_mpi -in in.flow.couette_for_run > log/log.lammps_$temp
...
curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendVideo -F chat_id=${CHATID} -F video="@couette.mp4" -F caption="Ovito visualization attached"
curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendMessage -d chat_id=${CHATID} -d text="The End"
